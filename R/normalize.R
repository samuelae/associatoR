...
ar_normalize_manual <- function(associations, fun, ..., process_cues = FALSE) {
  ...
  if (process_cues) {
    ...
    if(!"cue_original" %in% names(associations$cues)){
      associations$cues$cue_original = associations$cues$cue
      associations$responses$cue_original = associations$responses$cue
      }
  ...
  if(!"response_original" %in% names(associations$responses)){
    associations$responses$response_original = associations$responses$response
    }
...
#'
#' \code{ar_normalize_dict} offers an interface to normalization responses (and cues) in \code{associatoR} objects based on a dictionary.
#'
#' @param associations an \code{associatoR} object containing association data as generated by \link[associatoR]{ar_import}.
#' @param dictionary a \code{data.frame} consisting of two columns named \code{c("old","new")}.
#' @param process_cues a \code{logical} indicating if cues should be processed (i.e. changed by \code{fun}) or not.
#'
#' @return Returns an \code{associatoR} object containing a list of tibbles:
#'
...
ar_normalize_dict <- function(associations, dictionary, process_cues = FALSE) {
  ...
  apply_dict = function(strings, dict){
    ifelse(strings %in% names(dict), dict[strings], strings)
    }
  ...
  # process responses
  associations$responses$response = apply_dict(associations$responses$response, dictionary)
