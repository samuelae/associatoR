...
#' @param protect_cues a \code{logical} indicating if cues should be protected (i.e. not changed by \code{fun}) or not.
#'
...
#' @examples
#'
#' ai_asso <- ai_asso %>% ar_normalize_manual(tolower)
#' ai_asso <- ai_asso %>% ar_normalize_manual(trimws, which = "left")
#'
#'@export

ar_normalize_manual <- function(associations, fun, ..., protect_cues = TRUE) {

  # checks
  chk::chk_s3_class(associations, "associatoR")
  chk::chk_function(fun)
  chk::chk_logical(protect_cues)

  # normalize ----

  # process cues
  unprotected = !associations$responses$cue %in% associations$cues$cue
  associations$responses$response[unprotected] = fun(associations$responses$response[unprotected], ...)
