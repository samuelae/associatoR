...
ar_word_cloud <- function(associations, facet_by = cluster, color_by = cluster, top_n = 30, ...) {
  ...
  plot = counts %>%
    ggplot2::ggplot(mapping = aes(label = response, size = n, color = !!color_by)) +
      ...
      ggplot2::theme_void() +
      ggplot2::facet_wrap(dplyr::enquo(facet_by))
