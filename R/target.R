...
#'
#' \code{ar_set_target} defines the analysis targets (cues and/or responses). Potential NA (e.g. from NA responses) will be removed from targets.
#'
...
ar_set_targets <- function(associations,
  ...
  # remove potential NA value from targets
  if(any(is.na(target_vec))) {
    target_vec <- target_vec[!is.na(target_vec)]
    cli::cli_alert_info("{.emph NA} value removed from targets.")
  }
...
#' @param case_sensitive a \code{logical} specifying whether to the case of targets. Default is \code{FALSE}.
#'
...
ar_characterize_targets <- function(associations,
  ...
  chk::chk_s3_class(associations, "associatoR")
  chk::chk_subset("targets", names(associations))
  chk::chk_logical(case_sensitive)
  ...
  if(chk::vld_character(characteristics) & any(characteristics %in% c("valence", "arousal","dominance"))) {
    ...
    if(case_sensitive){
      ...
      associations$targets <- associations$targets %>%
        dplyr::mutate(target_lower = stringr::str_to_lower(target)) %>%
        dplyr::left_join(warriner %>% dplyr::mutate(word = stringr::str_to_lower(word)),
                         by = c("target_lower" = "word")) %>%
        dplyr::select(-target_lower)
      }
    }
  ...
  if(chk::vld_character(characteristics) & any(characteristics %in% c("concreteness", "word_frequency"))) {
    ...
    if(case_sensitive){
      ...
      associations$targets <- associations$targets %>%
        dplyr::mutate(target_lower = stringr::str_to_lower(target)) %>%
        dplyr::left_join(brysbaert %>% dplyr::mutate(word = stringr::str_to_lower(word)),
                         by = c("target_lower" = "word")) %>%
        dplyr::select(-target_lower)
      }
    }
...
#' Add response variable summaries to targets
#'
#' @param associations an \code{associatoR} object containing association data as generated by \link[associatoR]{ar_import}.
#' @param response_var a variable in the responses tibble of the \code{associatoR} object.
#' @param fun a function used to summarize response_vars for each target, defaults to \code{mean}.
#' @param ... additional parameters passed on to \code{fun}.
#'
#' @return Returns an \code{associatoR} object where \code{targets} gains an additional column including the target summary calculated from the \code{responses} table's \code{response_var} using \code{fun}.
#' @export
#'
#' @examples
#' ar_import(intelligence,
...
ar_summarize_targets <- function(associations, response_var, fun = mean, ...) {

  # handle input
  response_var <- dplyr::enquo(response_var)

  # checks
  check_object(associations)
  check_targets(associations)
  check_tidy(associations$responses, response_var, data_label = "responses")
  chk::chk_function(fun)

  # summarize
  target_summary <- associations$targets %>%
    dplyr::left_join(associations$responses, by = c("target" = "response")) %>%
    dplyr::group_by(target) %>%
    dplyr::summarize(fun(eval(response_var), ...))
  names(target_summary)[-1] <- rlang::as_name(response_var)

  # add to associations
  associations$targets <- associations$targets %>%
    dplyr::left_join(target_summary, by = "target")
